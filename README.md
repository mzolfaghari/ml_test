...
```bash
ml_test$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ml_test$ sam local start-api
ml_test$ curl http://localhost:3000/classify_digit
```
...
```bash
ml_test$ sam logs -n InferenceFunction --stack-name ml_test --tail
```
...
```bash
aws cloudformation delete-stack --stack-name ml_test
```
